...
def main(argv: list[str] = sys.argv[1:]):
    ...
    print("Raw ToT plot...")
    raw_tot_plot(histograms_data, output_dir/f"tot_raw.png", config_file)
...
def landaumvp_fit(histograms_data, output_file, config_file, calibration, calib_type = "log"):
    ...
    for (hist_name, (timestamps, hist_data)), (ax, bottom_ax) in zip(histograms_data.items(), zip(axs, bottom_axs)):
        ...
        if calib_type != "both":
            ax.plot(x_axis, popt2[0]*langauss.pdf(x_axis, popt2[1], popt2[2], popt2[3]), "-", color="red", label = "LandauXGauss MPV=%.2f±%.2f" % (popt2[1],perr2[1]))    
            ax.plot(x_axis, popt[0]*landau.pdf(x_axis, popt[1], popt[2]), "--", color="blue", label = "Landau MPV=%.2f±%.2f" % (popt[1],perr[1]))    
        else:
            ax.plot(x_axis, popt[0]*landau.pdf(x_axis, popt[1], popt[2]), "-", color="red", label = "[log-cal] MPV=%.2f±%.2f" % (popt[1],perr[1]))   
            x_axis2 = np.linspace(math.floor(min(fc_bins2)), max(fc_bins2), 1000)
            ax.plot(x_axis2, popt_landau_2[0]*landau.pdf(x_axis2, popt_landau_2[1], popt_landau_2[2]), linestyle=(0, (5, 10)), color="blue", label = "[spline-cal] MPV=%.2f±%.2f" % (popt_landau_2[1],perr_landau_2[1]))   
