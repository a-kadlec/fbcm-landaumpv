...
def get_board_calibration(config_file):
    ...
    side = "_"+config_file['measurement']
    ...
    for ch_num in test_conditions['asic_channels']:
        ...
        for th_level_fC in range(len(test_conditions['th_list_fC'])):
            for r in range(len(test_conditions['rccal'])):
                ...
                popt, pcov = curve_fit(tot_func, chcalval[start:end:step]*1e15, myTot*1e9, p0=config_file['log_p0'], sigma = tot_std[r,th_level_fC,start:end:step]*1e9, absolute_sigma = True)
                # , p0=(11,2,0.01,1)
                ...
                # plt.yscale('log')
                fig.savefig(config_file['output_dir']+'/calib_plots/tot_tw_sample_%s_%s_channel_%d_rccal_%d_th_%s.jpg' % (sample, file_num, ch_num, test_conditions['rccal'][r],  test_conditions['th_list_fC'][th_level_fC]), format='jpg', bbox_inches='tight')
                plt.close()
    return output
